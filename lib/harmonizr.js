...
function harmonize(src, options) {
    ...
    var ret = modifier.finish();
    return options.get_ast ? modifier.ast : ret;
}
        ...
        if (method.key.name === 'constructor') {
            ...
            if (method.kind) {
                ...
                modifier.replace(method.loc.start, method.key.loc.end,
                    'var __' + method.kind + prop + ' = ' +
                    'Object.getOwnPropertyDescriptor(' + protoprop + ');\n' +
                    'Object.defineProperty(' + protoprop + ', ' +
                    '{configurable: true, ' + otherkind + ': __' + method.kind + prop +
                    ' && __' + method.kind + prop + '.' + otherkind + ', ' +
...
// register harmonizr extension
if (require && require.extensions) {
    var fs = require('fs');
    require.extensions['.harmonizr'] = function(module, filename) {
        var content = harmonize(fs.readFileSync(filename, 'utf8'));
