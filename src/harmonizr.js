...
export function harmonize(src, options) {
    ...
    if (options.fail) {
        stages.unshift({name: 'fail', fn: (modifier) => {
...
function processClasses(modifier, options) {
    ...
    do {
        ...
        if (node.superClass) {
            ...
            if (hasGetterSetter()) {
                // insert method to get the property descriptor of any of the parents
                modifier.insert(node.superClass.loc.start,
                    'function __prop(O, P) { return O && (Object.getOwnPropertyDescriptor(O, P) || ' +
                    '__prop(Object.getPrototypeOf(O), P)) || {}; }');
            }
        ...
        if (method.key.name === 'constructor') {
            ...
            if (method.kind) {
                ...
                modifier.replace(method.loc.start, method.key.loc.end,
                    ...
                    '{configurable: true, ' + otherkind + ': ' +
                    '__prop(' + protoprop + ').' + otherkind + ', ' +
...
export var moduleStyles = {
    amd: {
        ...
        importModuleDeclaration(mod, imp, options) {
